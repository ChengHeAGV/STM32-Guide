...
u16 adc1[16];//Ä¬ÈÏ²ÎÊı
u16 adc2[16];
u16 tempmem[20];//ÄÚ´æ
u16 led[16];
u16 threshold=0;//ãĞÖµ
u8 Read_ID=0;
//CAN³õÊ¼»¯
...
void USB_LP_CAN1_RX0_IRQHandler(void)
{
    ...
    CAN_Receive(CAN1, 0, &RxMessage);
		...
		if(RxMessage.StdId==127)//µ±ÊÇ´Åµ¼º½ĞŞ¸ÄÆ÷µÄÊı¾İÀ´ÁÙÊ±
		{
			...
			{
				if(RxMessage.Data[1]==2) //ÉèÖÃ
				{
					Read_ID = RxMessage.Data[2];//¸üĞÂID
					WriteFlashOneWord(17,Read_ID);//Ğ´Èëflash
				}
			}
			//ãĞÖµ
			else if(RxMessage.Data[0]==3) 
			{
				if(RxMessage.Data[1]==2) //ÉèÖÃ
				{
					threshold = RxMessage.Data[2];//¸üĞÂãĞÖµ
					WriteFlashOneWord(18,threshold);//Ğ´Èëflash
				}
			}
		}
}
